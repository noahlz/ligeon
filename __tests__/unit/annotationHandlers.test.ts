...
describe('upsertAnnotation', () => {
  ...
  test('updates existing annotation at same ply (upsert semantics)', async () => {
    const first = await upsertAnnotation(TEST_COLLECTION, 1, 3, 1, tmpDir)
    expect(first).not.toBeNull()
    const second = await upsertAnnotation(TEST_COLLECTION, 1, 3, 2, tmpDir)
    expect(second).not.toBeNull()
    expect(second!.nag).toBe(2)
    // Only one annotation at ply 3
    const all = await getAnnotations(TEST_COLLECTION, 1, tmpDir)
    expect(all.length).toBe(1)
    expect(all[0].nag).toBe(2)
  })
...
describe('deleteAnnotation', () => {
  test('deletes annotation and it no longer appears in getAnnotations', async () => {
    await upsertAnnotation(TEST_COLLECTION, 1, 3, 1, tmpDir)
    const result = await deleteAnnotation(TEST_COLLECTION, 1, 3, tmpDir)
  ...
  test('only deletes annotation at specified ply', async () => {
    await upsertAnnotation(TEST_COLLECTION, 1, 3, 1, tmpDir)
    await upsertAnnotation(TEST_COLLECTION, 1, 5, 2, tmpDir)
    await deleteAnnotation(TEST_COLLECTION, 1, 3, tmpDir)
    const remaining = await getAnnotations(TEST_COLLECTION, 1, tmpDir)
  ...
  test('is idempotent — deleting non-existent annotation returns success', async () => {
    const result = await deleteAnnotation(TEST_COLLECTION, 1, 99, tmpDir)
    expect(result.success).toBe(true)
  })

  test('returns false for invalid ply (zero)', async () => {
    const result = await deleteAnnotation(TEST_COLLECTION, 1, 0, tmpDir)
    expect(result.success).toBe(false)
  })

  test('returns false for invalid collectionId', async () => {
    const result = await deleteAnnotation('', 1, 3, tmpDir)
    expect(result.success).toBe(false)
  })

  test('returns false for invalid gameId', async () => {
    const result = await deleteAnnotation(TEST_COLLECTION, 0, 3, tmpDir)
    expect(result.success).toBe(false)
  })
})
